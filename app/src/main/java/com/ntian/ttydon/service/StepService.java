package com.ntian.ttydon.service;
...
import com.ntian.ttydon.activity.MainActivity;
import com.ntian.ttydon.config.StepContant;
import com.ntian.ttydon.entity.StepData;
import com.ntian.ttydon.util.DbUtils;
...
/**
 * Created by mjt89 on 2017/9/16 0016.
 */
...
public class StepService extends Service implements SensorEventListener {
    ...
    private class MessenerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case StepContant.MSG_FROM_CLIENT:
                    try {
                        Messenger messenger = msg.replyTo;
                        Message replyMsg = Message.obtain(null, StepContant.MSG_FROM_SERVER);
    ...
    private void initTodayData() {
        ...
        DbUtils.getLiteOrm().setDebugged(true);
        // 获取当天的数据，用于展示
        List<StepData> list = DbUtils.getQueryByWhere(StepData.class, "today",
...
//                    Log.d("重置步数" + StepDcretor.CURRENT_SETP);
                    save();
                    isNewDay();
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_SETP);
    ...
    private void isNewDay() {
        ...
        if (time.equals(new SimpleDateFormat("HH:mm").format(new Date())) ||
                !CURRENTDATE.equals(getTodayDate())) {
            initTodayData();
        }
    }
     ...
     */
    public void isCall() {
        String time = this.getSharedPreferences("share_date", Context.MODE_MULTI_PROCESS)
                .getString("achieveTime", "21:00");
        String plan = this.getSharedPreferences("share_date", Context.MODE_MULTI_PROCESS)
                .getString("planWalk_QTY", "7000");
        String remind = this.getSharedPreferences("share_date", Context.MODE_MULTI_PROCESS)
                .getString("remind", "1");
//        Logger.d("time="+time+"\n"+
...
//            workPlanNotification();
    ...
    private void updateNotification(String content) {
        ...
        builder.setTicker("TTYDon计步");
        builder.setContentTitle("TTYDon计步");
        // 设置不可清除
    ...
    private void workPlanNotification() {
        ...
        builder.setContentIntent(contentIntent);
        builder.setSmallIcon(R.mipmap.ic_launcher);
        builder.setTicker("TTYDon计步提醒您开始锻炼了");
...
//				.setNumber(number)//显示数量
                .setTicker("TTYDon计步提醒您开始锻炼了")//通知首次出现在通知栏，带上升动画效果的
